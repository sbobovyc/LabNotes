...
class GraphFrame(wx.Frame):
    ...
    def init_plot(self):
        ...
        self.axes.set_title('Put titile here', size=12)
        ...
        self.fig.legend([self.plot_data, self.plot_data2,self.plot_data3], ["Right to Left", "Right to Back", "Left to Back"], prop={'size':10})
    ...
    def get_data(self):
        ...
        token = '$$$'
        len_packet = 105 # in bytes
        new_data = self.datagen.next()
        
        if new_data != None and len(new_data) > 0:
            self.raw_data += new_data
            index = self.raw_data.find(token)
            if index != -1:
                #print "Found at ", index
                substring = self.raw_data[index+len(token):]
                #print "Length of string", len(substring)
                if len(substring) >= len_packet:
                    #print binascii.hexlify(substring[0:len_packet])
                    data = struct.unpack(">26I1B", substring[0:len_packet])
                    self.raw_data = substring[len_packet+1:]
                    print "Got ", data
                    while self.datagen.queue.empty()==False:
                       self.datagen.queue.get_nowait() #empty queue quickly  
                    return data
                else:
                    return None
        return None
        ...
        if not self.paused:
            try:
                ...
                if data != None:
                    ...
                    if (movement == 1): #0001
                        app.frame.SetTitle("Right")
                        self.set_background_color('g')  #right
                        
                    elif (movement == 2): #0010
                        app.frame.SetTitle("Left")
                        self.set_background_color('m')  #left

                    elif (movement == 4): #0100
                        app.frame.SetTitle("Back")
                        self.set_background_color('k')  #back

                    elif (movement == 5): #0101
                        app.frame.SetTitle("Right and back")
                        self.set_background_color('b')  #right and back

                    elif (movement == 6): #0110
                        app.frame.SetTitle("Left and back")
                        self.set_background_color('y')  #left and back
                            
                    elif (movement == 8): #1000
                        app.frame.SetTitle("Front")
                        self.set_background_color('w')  #front

                    elif (movement == 9): #1001
                        app.frame.SetTitle("Right and front")
                        self.set_background_color('r') #right and front
                        
                    elif (movement == 10): #1010
                        app.frame.SetTitle("Left and front")
                        self.set_background_color('c') #left and front

                    else:
                        app.frame.SetTitle("No Move")
                        self.set_background_color('0.75')  #nothing                    
...
if __name__ == '__main__':
    ...
    parser.add_argument('--port', nargs='?', default="COM23", help='Serial port')
    parser.add_argument('--baud', nargs='?', default=9600, help='Baud rate')
    parser.add_argument('--time', nargs='?', default=.0000000, help='Refresh time in ms')
    ...
    app.frame = GraphFrame(args.port, args.baud)
